...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "recipient", help="Email address to send the test message to"
        )
    ...
    def handle(self, *args, **options):
        ...
        try:
            sent = send_mail(
                subject, message, from_email, [recipient], fail_silently=False
            )
        ...
        if sent:
            self.stdout.write(
                self.style.SUCCESS(f"Письмо поставлено в очередь → {recipient}")
            )
    ...
    def _show_queue_stats(self):
        try:
            ...
            self.stdout.write(
                f"Очередь mailer: ожидает отправки — {pending}, "
