...
- [x] `src/apps/mobile/README.md`: строка в таблице эндпоинтов и в схеме прав рядом с `tags/`; заметка про общий бакет `mobile-write`
- [x] переместить план в `docs/plans/completed/` (выполняет оркестратор после ревью — ещё не перемещён)
