...
- [x] `RaceForm.promos_json = CharField(required=False, widget=HiddenInput)` (пустой payload —
      не ошибка, как у доп-услуг)
- [x] `_validate_promo_rows`: `code` `^[A-Z0-9_-]{2,32}$` (в верхний регистр), уникален в гонке;
      `discount_type` из двух значений; `value` — percent `1…100`, fixed `> 0`; `max_uses ≥ 0`;
      `comment` ≤ 255
- [x] `_reconcile_promos`: матч по `id`, затем по `code`; `order = index`; отсутствующая строка
...
- [x] `code` read-only после сохранения (правка кода существующей строки игнорируется)
- [x] `_existing_promos(race)` для контекста: поля строки + `used` (из `occupied_team_ids`) и
      `max_uses`; сейчас это один запрос на промокод — оставить комментарий об N+1 (кодов единицы)
- [x] подключить парсинг/валидацию/реконсайл в `RaceEditView.post` внутри существующего
...
- [x] тесты: создание, правка, деактивация используемой строки, удаление неиспользуемой,
      попытка сменить `code` сохранённой строки, все ветки валидации, ошибки не сохраняют ничего
- [x] прогнать тесты — должны пройти до перехода к задаче 12
...
- [x] секция «Промокоды» после доп-услуг: заголовок, заголовки колонок, контейнер строк,
      кнопка добавления, скрытый `promos_json`, JSON-остров `#promos-data`
- [x] в `race_form.js` — построение строк из острова, добавление/удаление/деактивация,
      сериализация в `promos_json` перед сабмитом (по образцу блока доп-услуг)
- [x] селектор типа скидки меняет подпись значения («%» / «₽»), клиентская валидация диапазонов
- [x] read-only колонка «использовано N из M» (`M` = «∞» при `max_uses = 0`), `code` заблокирован
      у сохранённых строк
- [x] стили секции в `race_form.css` в стиле соседних блоков
- [x] тесты вьюхи: страница рендерит остров `#promos-data` с ожидаемыми строками и счётчиком `used`
- [x] прогнать тесты — должны пройти до перехода к задаче 13

### Task 13: Verify acceptance criteria

- [x] сквозной сценарий на локальном стенде: создать промокод → зарегистрировать команду со скидкой
      → подтвердить платёж → повторная попытка того же кода той же командой даёт `already_used`
- [x] проверить исчерпание лимита разными командами и освобождение квоты по истечении 20 мин
- [x] проверить 100%-скидку: платёж `done` с суммой 0, места и доп-услуги зачислены
- [x] проверить, что без промокода все прежние пути (оплата, доплата, нулевой заряд) не изменились
- [x] `make format && make lint` — чисто
- [x] полный прогон `uv run pytest` — зелёный

### Task 14: [Final] Update documentation

- [x] добавить в `CLAUDE.md` секцию «Промокоды» рядом с «Team add-ons / доп-услуги»: модель,
...
- [x] отразить в `CLAUDE.md` переезд зачисления в `apps/race/settlement.py:settle_payment`
- [x] перекрёстная ссылка на промокоды в шапке `src/apps/race/pricing.py`
