...
- [ ] читать оба JSON-острова, при их отсутствии — тихо выйти (как в `teams.js`)
- [ ] реализовать фильтр статуса (оплачено / не оплачено / возвраты / все, дефолт «оплачено»)
      и поиск по названию команды и `order_id`
- [ ] реализовать сортировку по клику на заголовок колонки (по дате — через `paid_sort`),
      дефолт — дата вниз
- [ ] рисовать тело таблицы из данных в пустой `<tbody>`
- [ ] считать плитки итогов под текущим фильтром (Собрано, Возвращено, Итого, Платежей,
      Участников оплачено, Скидок, Средний чек)
- [ ] считать разбивку дохода: «Участие», строка на каждый `RaceExtra` из каталога,
      «Скидка», «Итого»
- [ ] считать динамику по дням по `paid_date` и рисовать CSS-бары (`width` % от максимума)
- [ ] стили в `payments.css`, всё под `.race-payments`; таблица в контейнере с
      `overflow-x: auto`, проверить ширину ~400px
- [ ] JS-тестов в проекте нет — проверить в браузере вручную (см. Post-Completion)
- [ ] прогнать `uv run pytest src/apps/race/tests.py` — ничего не сломалось
...
- [ ] добавить в `finance.py` `csv_rows(rows, extras)` — плоские строки с отдельной колонкой
      на каждую услугу каталога
- [ ] добавить `filter_rows(rows, status)` (`done` / `unpaid` / `cancel` / `all`, дефолт `done`,
      неизвестное значение → `done`) — **только для экспорта**: страница отдаёт все строки и
      фильтрует в JS
- [ ] добавить `RacePaymentsExportView`, переиспользуя помощник гейта из задачи 2; отдавать
      `text/csv; charset=utf-8`, BOM `﻿`, разделитель `;`, имя `payments-<slug>-<YYYY-MM-DD>.csv`
- [ ] зарегистрировать путь `race/<slug:race_slug>/payments/export/` name=`race_payments_export`
- [ ] написать тесты: 200 для `RaceAdmin(ADMIN)`, 403 для постороннего, BOM в начале тела,
      число строк соответствует `?status=done`
- [ ] написать тест: неизвестный `?status=` откатывается к `done`, не падает
- [ ] запустить тесты — должно пройти до задачи 6
